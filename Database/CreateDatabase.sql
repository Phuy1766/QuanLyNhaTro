...
CREATE TABLE BAOTRI_TICKET (
    ...
    PhongId INT NOT NULL,
    KhachId INT,
    LoaiSuCo NVARCHAR(50), -- Điện, Nước, Thiết bị, Cửa/Khóa, Điều hòa, Khác
...
GO
-- =====================================================
-- Migration Script: Add LoaiSuCo column to BAOTRI_TICKET
-- Date: 2025-11-22
-- Description: Adds LoaiSuCo (Issue Type) column for tenant maintenance requests
-- =====================================================
...
IF NOT EXISTS (SELECT 1 FROM sys.columns
               WHERE object_id = OBJECT_ID('BAOTRI_TICKET')
               AND name = 'LoaiSuCo')
BEGIN
    ALTER TABLE BAOTRI_TICKET
    ADD LoaiSuCo NVARCHAR(50) NULL;

    PRINT 'Column LoaiSuCo has been added to BAOTRI_TICKET table successfully.';
END
ELSE
BEGIN
    PRINT 'Column LoaiSuCo already exists in BAOTRI_TICKET table.';
END
GO

-- Update existing records with default value
UPDATE BAOTRI_TICKET
SET LoaiSuCo = N'Khác'
WHERE LoaiSuCo IS NULL;
GO
...
-- Thêm trạng thái "ChoXacNhan" cho tính năng thanh toán online
-- =====================================================
...
-- Xóa constraint cũ
IF EXISTS (SELECT * FROM sys.check_constraints WHERE name = 'CK_HoaDon_TrangThai')
BEGIN
    ALTER TABLE HOADON DROP CONSTRAINT CK_HoaDon_TrangThai;
    PRINT 'Đã xóa constraint cũ CK_HoaDon_TrangThai';
END
GO

-- Thêm constraint mới với trạng thái "ChoXacNhan"
ALTER TABLE HOADON
ADD CONSTRAINT CK_HoaDon_TrangThai
CHECK (TrangThai IN (N'ChuaThanhToan', N'DaThanhToan', N'QuaHan', N'ChoXacNhan'));
GO

PRINT 'Đã cập nhật constraint với trạng thái mới: ChoXacNhan';
GO
